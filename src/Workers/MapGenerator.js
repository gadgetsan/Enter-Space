...
self.addEventListener('message', function(e) {
    ...
    precision = e.data[0];
    chunkStartX = e.data[1];
    chunkStartY = e.data[2];
    smoothSize = e.data[3];
    chunkHeight = e.data[4];

    GetHeight = function(x, z, precision){
        var actualx = (chunkStartX+(x*precision));
        var actualz = (chunkStartY+(z*precision));
        var scale = 500;
    ...
    for(var x=0; x<size+1; x++){
        ...
        for(var z=0; z<size+1; z++){
            heightMap[x].push(GetHeight(x, z, precision));
        }
    }
    //console.dir(heightMap);
    var chunkTower=[];
    for(var hc=0; hc<chunkHeight;hc++){
        var map = [];
